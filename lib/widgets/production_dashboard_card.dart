  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        final double total = _provider.totalCost(widget.season.id).toDouble();
        final double hectares = widget.season.plannedArea / 10000;
        final double perHa = hectares > 0 ? total / hectares : 0.0;
        final groups = <String, double>{};
        for (final log in logs) {
          groups[log.activityType] = (groups[log.activityType] ?? 0.0) + log.cost;
        }
              ...
              children: [
                ...
                if (top.isNotEmpty) ...[
                  ...
                  ...top.take(5).map((entry) {
                    final double ratio = total > 0 ? entry.value / total : 0.0;
                    return Padding(
