...
class _FieldMapScreenState extends State<FieldMapScreen> {
  ...
  List<FieldModel> _filteredFields(List<FieldModel> fields) {
    final query = _searchController.text.trim().toLowerCase();
    return fields.where((field) {
      final matchesSearch = query.isEmpty ||
          field.id.toLowerCase().contains(query) ||
          field.name.toLowerCase().contains(query);
      final matchesCrop = _cropFilter == 'Tất cả' || field.crop == _cropFilter;
      final matchesStatus = _statusFilter == 'Tất cả' || field.status == _statusFilter;
    ...
    return Scaffold(
      ...
      body: AnimatedBuilder(
        ...
        builder: (context, _) {
          ...
          final fields = _filteredFields(allFields)
                  ...
                  child: SafeArea(
                    child: Card(
                      child: ListTile(
                        ...
                        isThreeLine: true,
                        trailing: IconButton(
                          tooltip: 'Chỉnh ranh giới',
                          onPressed: () => _editSelectedField(selected),
  ...
  Future<void> _showFilters() async {
    ...
    final crops = <String>{
      'Tất cả',
      ...fields.map((f) => f.crop).where((v) => v.isNotEmpty),
    }.toList();
    final statuses = <String>{
      'Tất cả',
      ...fields.map((f) => f.status).where((v) => v.isNotEmpty),
    }.toList();
  ...
  Future<void> _showMeasureTools() async {
    await showModalBottomSheet<void>(
      context: context,
      showDragHandle: true,
      builder: (_) => SafeArea(
            ...
            children: [
              ...
              ListTile(
                ...
                onTap: () {
                  ...
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const FieldManualMeasureScreen()));
                },
              ),
              ListTile(
                ...
                onTap: () {
                  Navigator.pop(context);
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const FieldGpsMeasureScreen()));
                },
              ),
              ListTile(
                leading: const Icon(Icons.straighten),
                title: const Text('Đo khoảng cách'),
                onTap: () {
                  Navigator.pop(context);
              ...
              if (_selectedId != null)
                ListTile(
                  ...
                  title: const Text('Chỉnh sửa ranh giới thửa đang chọn'),
                  onTap: () {
                    final field = _provider.getFieldById(_selectedId!);
                    Navigator.pop(context);
                    if (field != null) _editSelectedField(field);
                  },
                ),
            ],
          ),
  ...
  Future<void> _editSelectedField(FieldModel field) async {
    await Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => FieldPolygonEditScreen(field: field)),
    );
  }
