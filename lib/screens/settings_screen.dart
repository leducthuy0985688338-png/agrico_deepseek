...
class SettingsPage extends StatelessWidget {
    ...
    return Scaffold(
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          const Text(
            'Khôi phục danh mục vật tư và số lượng tồn kho từ Cloud.',
            style: TextStyle(color: Colors.grey, fontSize: 12),
            textAlign: TextAlign.center,
          ),
          const SizedBox(height: 12),
          ...
          const SizedBox(height: 8),
          const Text(
            'Khôi phục vật tư, nhân công, máy móc và nhiên liệu sản xuất.',
            style: TextStyle(color: Colors.grey, fontSize: 12),
            textAlign: TextAlign.center,
          ),
          const SizedBox(height: 12),
  ...
  Widget _buildRestoreWarehouseButton(BuildContext context) {
    return Consumer<CloudSyncProvider>(
      builder: (context, provider, child) {
        return OutlinedButton(
          onPressed: provider.isBusy
              ? null
              : () => _confirmWarehouseRestore(context),
          style: OutlinedButton.styleFrom(
            foregroundColor: AppTheme.primaryColor,
            side: const BorderSide(color: AppTheme.primaryColor),
            padding: const EdgeInsets.symmetric(vertical: 14),
          ...
          child: provider.isRestoringWarehouse
              ? const Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    SizedBox(
                    ...
                    SizedBox(width: 12),
                    Text('Đang khôi phục kho vật tư...'),
                  ],
                )
              : const Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    ...
                    SizedBox(width: 8),
                    Text('Khôi phục kho vật tư từ Cloud'),
                  ],
                ),
        );
      },
  ...
  Future<void> _confirmWarehouseRestore(BuildContext context) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (dialogContext) => AlertDialog(
        title: const Text('Khôi phục kho vật tư?'),
        content: const Text(
          'Danh mục vật tư và số lượng tồn kho hiện tại sẽ được thay bằng '
          'bản trên Cloud.',
        ),
        actions: [
          TextButton(
    ...
    try {
      final items = await cloudProvider.restoreWarehouseData();
      if (!context.mounted) return;
      ...
      if (items.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Cloud chưa có dữ liệu kho vật tư. '
              'Dữ liệu cục bộ được giữ nguyên.',
            ),
          ),
        );
      ...
      final restoredCount =
          warehouseProvider.restoreFromCloud(items: items);
      if (restoredCount == 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Cloud không có vật tư hợp lệ. '
              'Dữ liệu cục bộ được giữ nguyên.',
            ),
          ),
        );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            '✅ Đã khôi phục $restoredCount vật tư và số lượng tồn kho.',
          ),
          backgroundColor: Colors.green,
        ),
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('❌ Lỗi khôi phục kho vật tư: $e'),
          backgroundColor: Colors.red,
        ),
      );
    }
  ...
  Widget _buildRestoreProductionCostButton(BuildContext context) {
    return Consumer<CloudSyncProvider>(
      builder: (context, provider, child) {
        return OutlinedButton(
          onPressed: provider.isBusy
              ? null
              : () => _confirmProductionCostRestore(context),
          style: OutlinedButton.styleFrom(
            foregroundColor: AppTheme.primaryColor,
            side: const BorderSide(color: AppTheme.primaryColor),
            padding: const EdgeInsets.symmetric(vertical: 14),
          ...
          child: provider.isRestoringProductionCosts
              ? const Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    SizedBox(
                    ...
                    SizedBox(width: 12),
                    Text('Đang khôi phục chi phí sản xuất...'),
                  ],
                )
              : const Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    ...
                    SizedBox(width: 8),
                    Text('Khôi phục chi phí sản xuất từ Cloud'),
                  ],
                ),
        );
      },
  ...
  Future<void> _confirmProductionCostRestore(BuildContext context) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (dialogContext) => AlertDialog(
        title: const Text('Khôi phục chi phí sản xuất?'),
        content: const Text(
          'Toàn bộ chi phí sản xuất hiện tại sẽ được thay bằng bản trên '
          'Cloud. Hãy khôi phục lô đất và mùa vụ trước để giữ đúng liên kết.',
        ),
        actions: [
          TextButton(
    ...
    try {
      final records = await cloudProvider.restoreProductionCostData();
      if (!context.mounted) return;

      if (records.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Cloud chưa có dữ liệu chi phí sản xuất.'),
          ),
        );
        return;
      }
      ...
      if (validRecords.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Không có chi phí nào khớp với lô đất và mùa vụ hiện tại. '
              'Hãy khôi phục lô đất, rồi khôi phục mùa vụ trước.',
            ),
          ),
        );
      ...
      await costProvider.restoreFromCloud(records: validRecords);
      if (!context.mounted) return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            '✅ Đã khôi phục ${validRecords.length} khoản chi phí sản xuất.'
            '$skippedMessage',
          ),
          backgroundColor: Colors.green,
        ),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('❌ Lỗi khôi phục chi phí sản xuất: $e'),
          backgroundColor: Colors.red,
        ),
      );
    }
